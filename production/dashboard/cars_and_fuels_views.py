...
{% for id, name, data, inv_id, inv_data, timestamp in cur %}
    <tr>
...
def view_car(id):
    ...
    cur = conn.cursor()
    cur.execute(
        'SELECT id, name, data, invocation_id, timestamp FROM cars WHERE id = %s',
        [id])
    [car_id, car_name, car_data, inv_id, timestamp] = cur.fetchone()
    cur.execute(
