...
def snake_path_gen(a, b):
    ...
    # we consider the bottom as non acceptable
...
def agent_gen(m: 'Model', start: 'Pos', a: 'Pos', b: 'Pos'):
    for x in navigate(start, Pos(a.x, b.y, a.z)):
        yield x
...
def go_to_start(a, b):
    def go(src):
        return navigate_pos(src, Pos(a.x, a.y, a.z))
...
# Copy paste
class DefaultSolver2DecLow(Solver):
    def __init__(self, args):
        self.w, self.h = map(int, args)

    def scent(self) -> str:
        return f'Default Dec low 2.4.4-{self.w}x{self.h}'
...
if __name__ == '__main__':
    ...
    with open('/home/behemoth/Work/icfpc2018-tbd/tasksF/FA032_tgt.mdl', 'rb') as f:
        data = f.read()
    m    = Model.parse(data)
